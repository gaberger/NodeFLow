		...
		var obj = {
		    ...
		    switch (type) {
	                    case 'OFPT_HELLO': 
			                ...
			                eventer.emit('OFPT_HELLO', socket, obj)  
			                break;
			            case 'OFPT_ERROR':
							mcounters.OFPT_ERROR++ 
			                eventer.emit('OFPT_ERROR', socket, obj);    
			                break;
			            case 'OFPT_ECHO_REQUEST':
							mcounters.OFPT_ECHO_REQUEST++ 
			                eventer.emit('OFPT_ECHO_REQUEST', socket, obj);
			                break;
			            case 'OFPT_PACKET_IN':
							mcounters.OFPT_PACKET_IN++
					        eventer.emit('OFPT_PACKET_IN', socket, obj) 
					
			                break;
						case 'OFPT_FEATURES_REPLY':
							mcounters.OFPT_FEATURES_REPLY++
			                eventer.emit('OFPT_FEATURES_REPLY', socket, obj)
			               break;
			            default:
			                debugger
