...
NodeFlowServer.prototype._processMessage = function(obj, sessionID) {
    ...
    if (obj.hasOwnProperty('message')) {
        ...
        case 'OFPT_HELLO':
            obj.outmessage = nfutils.setSyncmessage('OFPT_HELLO', obj)
            nfutils.sendPacket(type, obj.outmessage, sessionID)
            obj.outmessage = nfutils.setSyncmessage('OFPT_FEATURES_REQUEST', obj)
            nfutils.sendPacket(type, obj.outmessage, sessionID)
            break;
        case 'OFPT_ERROR':
            // TODO
        ...
        case 'OFPT_ECHO_REQUEST':
            obj.outmessage = nfutils.setSyncmessage('OFPT_ECHO_REPLY', obj)
            nfutils.sendPacket(type, obj.outmessage, sessionID)
            break;
        case 'OFPT_PACKET_IN':
...
NodeFlowServer.prototype._forward_l2_packet = function(obj, packet) {
    ...
    if ((dl_dst != 'ff:ff:ff:ff:ff:ff') && l2table[dpid].hasOwnProperty(dl_dst)) {
        ...
        if (prt == in_port) {
            ...
            self.emit('SENDPACKET', { "packet": obj, "type": 'OFPT_PACKET_OUT'})

        } else {
            out_port = prt
            var outtxt = util.log(util.format("Installing flow for destination: %s source: %s  in_port: %s\tout_port: %s\tsystem=NodeFlow", dl_dst, dl_src, in_port, out_port))
            obj.outmessage = nfutils.setFlowModPacket(obj, packet, in_port, out_port)
            self.emit('SENDPACKET', { "packet": obj, "type": 'OFPT_FLOW_MOD'})
            // nfutils.sendPacket('OFPT_FLOW_MOD', obj.outmessage, obj.sessionID)
